    ...
    while True:
        try:
            ...
            path_to_chromedriver = os.path.normpath(
                os.path.join(os.getcwd(), webdriver_folder_name,
                             "chromedriver.exe")
            )
            if activate_tor:
            ...
            identifier = driver.find_element_by_xpath(
                '//*[@id="identifierId"]')
            delay()
            identifier.send_keys("seleniums541@gmail.com"+Keys.ENTER)
            delay()
            password = driver.find_element_by_xpath(
                '//*[@id="password"]/div[1]/div/div[1]/input')
            delay()
            password.send_keys("Seleniumroot", Keys.ENTER)
            delay()
            # go to website
